<template>
  <popup @close="$emit('close')" :isLoading="isLoading" :shown="shown">
    <template v-slot:header>Импорт промежуточной явки</template>
    <template v-slot:body>
      <alert :messages="errors.length ? errors : message"></alert>
      <file
        :linkToExample="asset('storage/upload_layouts/intermediate_official_example.csv')"
        v-model="data.file"
      >Файл для импорта
      </file>
    </template>
  ...
  export default {
    ...
    methods: {
      ...
      upload() {
        ...
        this.$store.dispatch('ViewCheck/uploadIntermediateVote', { check_id, file }).then(res => {
