  ...
  <div>
    ...
    >
      ...
      <template v-slot:status="props">
        <ul class="uik-type-list">
          <li class="uik-type-list__item">
            <sign-status :status="props.row.status"></sign-status>
          </li>
        </ul>
      </template>
  ...
  methods: {
    clickModal(props) {
      ...
      this.violation = Array.isArray(props.key)
        ? props.key.map(item => String(item))
        : String(props.key);
      this.uik = String(props.uik.id);
      this.camera = String(props.camera);
      this.shown = true;
    }
  },
  computed: {
    table() {
      ...
      const data = violation.map((item, i) => {
        ...
        return {
          key: Array.isArray(item.id) ? item.id.sort() : item.id,
          date: date,
          time: time,
